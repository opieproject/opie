...
} Q_PACKED MMD0BLOCK;		// BYTE data[lines+1][tracks][3];
...
} Q_PACKED MMDCOMMAND;  // Last entry has offset == 0xFFFF, cmd_number == 0 and 0 extrabytes
...
//---------------------------------------------------------------
{
	...
	case 0x0F:	// Set Tempo / Special
		...
		if (param <= 0xF0)
		{
			...
			{
				param = (param > 9) ? 99 : bpmvals[param-1];
			} else
			// F.01 - F.0A: Set Speed
			if (param <= 0x0A)
			{
