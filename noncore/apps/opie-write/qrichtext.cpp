...
{
    if ( !doc ) {
        owarn << "no document available" << oendl;
        return 0;
    }
    if ( !c ) {
        owarn << "handed empty text cursor" << oendl;
        return 0;
    }

    QTextParagraph *s = doc->paragAt( id );
    if ( !s ) {
...
void QTextCursor::gotoNextWord()
{
    ...
    QTextString *s = para->string();
    bool allowSame = false;
    for ( int i = idx; s && i < (int)s->length(); ++i ) {
	if ( ! (s->at( i ).c.isSpace() || s->at( i ).c == '\t' || s->at( i ).c == '.' ||
	     s->at( i ).c == ',' || s->at( i ).c == ':' || s->at( i ).c == ';') )
        {
	    if ( !allowSame )
