/****************************************************************************
** $Id: qcolordialog.cpp,v 1.1 2003-06-14 17:39:27 llornkcor Exp $
**
** Implementation of KColorDialog class
**
** Created : 990222
**
...
/*!
  ...
  KColorDialog. All color dialogs share the same custom colors.
*/
int KColorDialog::customCount()
{
    return 2*8;
}
 ...
 */
QRgb KColorDialog::customColor( int i )
{
    initRGB();
    if ( i < 0 || i >= customCount() ) {
#ifdef CHECK_RANGE
	qWarning( "KColorDialog::customColor() index %d out of range", i );
#endif	
	i = 0;
    }
    return cusrgb[i];
...
*/
void KColorDialog::setCustomColor( int i, QRgb c )
{
    initRGB();
    if ( i < 0 || i >= customCount() ) {
#ifdef CHECK_RANGE
    ...
    :QWidget( parent, name)
{
    ...
    QLabel *l = new QLabel( hEd, KColorDialog::tr("Hu&e:"), this );
    ...
    l = new QLabel( sEd, KColorDialog::tr("&Sat:"), this );
    l->setAlignment( AlignRight|AlignVCenter );
    gl->addWidget( l, 1, 1 );
    ...
    l = new QLabel( vEd, KColorDialog::tr("&Val:"), this );
    l->setAlignment( AlignRight|AlignVCenter );
    gl->addWidget( l, 2, 1 );
    ...
    l = new QLabel( rEd, KColorDialog::tr("&Red:"), this );
    l->setAlignment( AlignRight|AlignVCenter );
    gl->addWidget( l, 0, 3 );
    ...
    l = new QLabel( gEd, KColorDialog::tr("&Green:"), this );
    l->setAlignment( AlignRight|AlignVCenter );
    gl->addWidget( l, 1, 3 );
    ...
    l = new QLabel( bEd, KColorDialog::tr("Bl&ue:"), this );
    l->setAlignment( AlignRight|AlignVCenter );
    gl->addWidget( l, 2, 3 );
    ...
    alphaLab = new QLabel( alphaEd, KColorDialog::tr("A&lpha channel:"), this );
...
class KColorDialogPrivate : public QObject
{
Q_OBJECT
public:
    KColorDialogPrivate( KColorDialog *p );
...
void KColorDialogPrivate::newHsv( int h, int s, int v )
{
    cs->setHsv( h, s, v );
    cp->setCol( h, s );
    lp->setCol( h, s, v );
...
void KColorDialogPrivate::setCurrentColor( QRgb rgb )
{
    cs->setRgb( rgb );
    newColorTypedIn( rgb );
}
...
void KColorDialogPrivate::newColorTypedIn( QRgb rgb )
{
    int h, s, v;
    rgb2hsv(rgb, h, s, v );
    cp->setCol( h, s );
...
void KColorDialogPrivate::newCustom( int r, int c )
{
    int i = r+2*c;
...
void KColorDialogPrivate::newStandard( int r, int c )
{
    setCurrentColor( stdrgb[r+c*6] );
    custom->setSelected(-1,-1);
}

KColorDialogPrivate::KColorDialogPrivate( KColorDialog *dialog ) :
    QObject(dialog)
{
    compact = FALSE;
	...
	QLabel * lab = new QLabel( standard,
				KColorDialog::tr( "&Basic colors"), dialog );
	...
	lab = new QLabel( custom, KColorDialog::tr( "&Custom colors") , dialog );
	leftLay->addWidget( lab );
	leftLay->addWidget( custom );

	QPushButton *custbut =
	    new QPushButton( KColorDialog::tr("&Define Custom Colors >>"),
						dialog );
    ...
    ok = new QPushButton( KColorDialog::tr("OK"), dialog );
    connect( ok, SIGNAL(clicked()), dialog, SLOT(accept()) );
    ok->setDefault(TRUE);
    cancel = new QPushButton( KColorDialog::tr("Cancel"), dialog );
	...
	QPushButton *addCusBt = new QPushButton(
					KColorDialog::tr("&Add To Custom Colors"),
						 dialog );
...
void KColorDialogPrivate::addCustom()
{
...
// BEING REVISED: jo
/*!
  \class KColorDialog qcolordialog.h
  \brief The KColorDialog class provides a dialog widget for specifying colors.
  \ingroup dialogs
    ...
    QDialog(parent, name, modal )
{
    ...
    d = new KColorDialogPrivate( this );
}
...
QColor KColorDialog::getColor( QColor initial, QWidget *parent,
			       const char *name )
{
    int allocContext = QColor::enterAllocContext();
    KColorDialog *dlg = new KColorDialog( parent, name, TRUE );  //modal
    if ( parent && parent->icon() && !parent->icon()->isNull() )
	dlg->setIcon( *parent->icon() );
    else if ( qApp->mainWidget() && qApp->mainWidget()->icon() && !qApp->mainWidget()->icon()->isNull() )
	dlg->setIcon( *qApp->mainWidget()->icon() );

    dlg->setCaption( KColorDialog::tr( "Select color" ) );
...
QRgb KColorDialog::getRgba( QRgb initial, bool *ok,
...
void KColorDialog::setColor( QColor c )
{
...
void KColorDialog::setSelectedAlpha( int a )
{
    d->showAlpha( TRUE );
    d->setCurrentAlpha( a );
}
...
int KColorDialog::selectedAlpha() const
{
